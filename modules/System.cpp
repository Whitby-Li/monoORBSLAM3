...
namespace mono_orb_slam3 {
    ...
    void System::Track(double timeStamp, cv::Mat grayImg, const vector<ImuData> &imus) {
        ...
        // recorder trajectory
        if (local_mapper->finishImuInit() && tracker->current_frame && trajectory_out.is_open()) {
    ...
    void System::setSaveFolder(const std::string &path) {
        ...
        if (!filesystem::exists(logFolder)) {
            cout << "create log folder: " << logFolder << endl;
            filesystem::create_directory(logFolder);
        }
        initial_logger.setNewPath(logFolder + "/initial.log");
        tracker_logger.setNewPath(logFolder + "/tracker.log");
        mapper_logger.setNewPath(logFolder + "/mapper.log");
#endif
    }
