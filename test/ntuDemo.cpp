...
int main(int argc, char *argv[]) {
    if (argc < 4) {
        cout << "Usage: ./ntu_demo setting.yaml vocabulary_file data_folder trajectory_save_path velo_bias_save_path" << endl;
        return -1;
    }
    ...
    while (idx1 < num_camera && idx2 < num_imu) {
        ...
        cv::Mat img = cv::imread(dataFolder + cv::format("/cam0/%08d.png", idx1), cv::IMREAD_GRAYSCALE);
        if (img.empty()) {
